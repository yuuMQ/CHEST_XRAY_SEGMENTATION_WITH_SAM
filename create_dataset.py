...
def create_dataset(data_path, output_path, category):
    ...
    os.makedirs(os.path.join(covid_output_path, "valid_images"))
    os.makedirs(os.path.join(covid_output_path, "valid_masks"))
    ...
    image_train_valid, image_test, mask_train_valid, mask_test = train_test_split(image_files, mask_files, test_size=0.1, random_state=42)
    image_train, image_valid, mask_train, mask_valid = train_test_split(image_train_valid, mask_train_valid, test_size=0.222, random_state=42)

    # train
    for image_file, mask_file in tqdm(zip(image_train, mask_train), total=len(image_train), desc="Copying train {}".format(category), colour='green'):
    ...
    # valid
    for image_file, mask_file in tqdm(zip(image_valid, mask_valid), total=len(image_valid), desc="Copying valid {}".format(category), colour='green'):
        shutil.copy(os.path.join(images_path, image_file), os.path.join(covid_output_path, "valid_images", image_file))
        shutil.copy(os.path.join(masks_path, mask_file), os.path.join(covid_output_path, "valid_masks", mask_file))

    # test
    for image_file, mask_file in tqdm(zip(image_test, mask_test), total=len(image_test), desc="Copying test {}".format(category), colour='green'):
